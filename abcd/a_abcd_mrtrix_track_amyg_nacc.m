function fibersPDB = a_abcd_mrtrix_track_amyg_nacc
...
subjects={};
file=fopen('/mnt/acorn/abcd/matproc/scripts/amyg_xform_makeup.txt','r'); %ran a and d this time
tline=fgetl(file);
while ischar(tline)
    disp(tline)
...
%subFiles = dir(fullfile(baseDir,'NDAR*'));
%subjects = {};
%for isubj =1:numel(subFiles)
%     lh_amyg_ROI = ['/mnt/acorn/abcd/matproc/' subFiles(isubj).name '/ROIs/lh_lateralamyg_fd.mat'];
%     rh_amyg_ROI = ['/mnt/acorn/abcd/matproc/' subFiles(isubj).name '/ROIs/rh_lateralamyg_fd.mat'];
%     lh_amyg_track = ['/mnt/acorn/abcd/matproc/' subFiles(isubj).name '/dti60trilin/fibers/mrtrix/' subFiles(isubj).name '_dwi_y1_aligned_trilin_csd_lmax6_lh_nacc_aseg_fd_lh_lateralamyg_fd_lh_nacc_aseg_fd_nonZero_MaskROI_lh_lateralamyg_fd_nonZero_MaskROI_union.nii.gz_lh_wmmask_amyg_fd_cut005_initcut005_curv05_step02_nfibers2000_prob.pdb'];
%     rh_amyg_track = ['/mnt/acorn/abcd/matproc/' subFiles(isubj).name '/dti60trilin/fibers/mrtrix/' subFiles(isubj).name '_dwi_y1_aligned_trilin_csd_lmax6_rh_nacc_aseg_fd_rh_lateralamyg_fd_rh_nacc_aseg_fd_nonZero_MaskROI_rh_lateralamyg_fd_nonZero_MaskROI_union.nii.gz_rh_wmmask_amyg_fd_cut005_initcut005_curv05_step02_nfibers2000_prob.pdb'];
%     if exist(lh_amyg_ROI, 'file') == 2 && exist(rh_amyg_ROI, 'file') == 2
%         if exist(lh_amyg_track, 'file') ~= 2 || exist(rh_amyg_track, 'file') ~= 2
...
%acpc has been redone for these subs, but none of the other preprocessing
%steps. Need to do that.
bad_acpc = {};
...
fclose(fileID);
subjects = setdiff(subjects, bad_acpc);

hemis = {'lh','rh'};

%for subjects who have a specific problem upon manual inspection. These
%will be skipped. Leave empty to not skip anyone.
bad_subs={};
            
for isubj = 1:length(subjects)
    
    if ~ismember(subjects{isubj},bad_subs)
        ...
        toRois   = {'_lateralamyg_fd'};
        wmMaskFS = '_amygnacc_merged_fd';       
        ...
        cutoff = 0.02; %FA cutoff along path
        initcutoff = 0.02; %FA cutoff at seed
