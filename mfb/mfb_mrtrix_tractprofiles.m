...
dirs    = 'dti30trilin';
logDir  = '/home/faberj/TEST/stats'; %where to save data?
baseDir = {'/home/faberj/TEST'}; %where subjects are in
     
subjects = {'SCA-BON-0083-0001'}; %subject names
                
% Set fiber groups
fiberName = {'DRTtract.mat', };
         
%% Set up the text file that will store the fiber vals.
dateAndTime     = getDateAndTime;
textFileName    = fullfile(logDir,['mfb_mrtrix_drttract_tp100',dateAndTime,'.csv']);
...
for i = 1:numel(baseDir)
    ...
    for ii=1:numel(subs)
        ...
        if ~isempty(sub)
            ...
            fiberDir = fullfile(subDir,dirs,'fibers/mrtrix');
            ...
            for kk=1:numel(fiberName)
                ...
                if exist(fiberGroup,'file')
                    ...
                    try
                        ...
                        [faTP,mdTP,rdTP,adTP] = mfb_dtiComputeDiffusionPropertiesAlongFG(fg, dt, 100);
                        avgfaTP = mean(faTP);
                        avgmdTP = mean(mdTP);
                        avgrdTP = mean(rdTP);
                        avgadTP = mean(adTP);
