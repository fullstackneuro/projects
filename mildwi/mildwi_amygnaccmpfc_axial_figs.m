...
for isubj = 1:length(subjects)
    ...
    hold on
    [fgh, lh] = mbaDisplayConnectome(lmpfc.fibers,fgh,[0.7,0.98,0.98],'single',[],[],0.3);
    delete(lh);
    [fgh, lh] = mbaDisplayConnectome(rmpfc.fibers,fgh,[0.7,0.98,0.98],'single',[],[],0.3);
    delete(lh);
    [fgh, lh] = mbaDisplayConnectome(lamyg.fibers,fgh,[0.23, 0.49, 0.63],'single',[],[],0.3);
    delete(lh);
    [fh, lh] = mbaDisplayConnectome(ramyg.fibers,fgh,[0.99,0.71,0.08],'single',[],[],0.3);
    delete(lh);
    view(0,55);
    %axis([-60 60 -50 90 -9 70]);
    lh = lightangle(0,50);
    %axis equal
    %save figure
    lfigurename = [subjects{isubj} '_amygnaccmpfc_ax'];
    feSavefig(fh,'figName',lfigurename,'figDir','/media/storg/matproc/figures/amygnacc/','figType','jpg')
end
