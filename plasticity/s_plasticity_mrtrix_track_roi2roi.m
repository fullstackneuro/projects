function [pdb_file, status, results] = s_plasticity_mrtrix_track_roi2roi(files, roi1, roi2, seed, mask, mode, n, n_max, bkgrnd, verbose)
...
switch mode_str
  case {'SD_PROB', 'SD_STREAM'}
   ...
   cmd_str = sprintf('streamtrack -seed %s -mask %s -include %s -include %s %s %s %s -number %d -maxnum %d -stop -minlength 10 -cutoff 0.075 -initcutoff 0.05 -curvature 1 -step 0.2',...
       seed,mask, roi1, roi2, mode_str, files.csd, tck_file, n, n_max);
