function [pdb_file, status, results] = s_lc_mrtrix_track_roi(files, roi1, seed, mask, mode, n, n_max, cutoff, initcutoff, curvature, stepsize, bkgrnd, verbose)
...
tck_file = strcat(strip_ext(files.csd), '_', strip_ext(roi1), '_',  ...
...
switch mode_str
  case {'SD_PROB', 'SD_STREAM'}
   ...
   cmd_str = sprintf('streamtrack -seed %s -mask %s -include %s %s %s %s -number %d -maxnum %d -stop -minlength 10 -cutoff %d -initcutoff %d -curvature %d -step %d',...
       seed, mask, roi1, mode_str, files.csd, tck_file, n, n_max, cutoff, initcutoff, curvature, stepsize);
