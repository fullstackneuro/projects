...
subs = {'els103','els106','els107','els111','els112','els113','els114', ...
    ...
    %{
        'els006','els009','els013','els014','els016','els017','els024', ...
        'els025','els026','els028','els032','els033','els034','els039', ...
        'els040','els041','els042','els045','els047','els049','els050', ...
        'els054','els055','els056','els058','els059','els060','els061', ...
        'els062','els064','els065','els068','els069','els070','els072', ...
        'els073','els074','els075','els076','els077','els079','els081', ...
        'els083','els085','els086','els087','els088','els090','els093', ...
        'els095','els097', ...
        %}
% Set fiber groups
fiberName = {'clean_rh_ains_nacc.mat','clean_lh_ains_nacc.mat'};

%% Set up the text file that will store the fiber vals.
dateAndTime     = getDateAndTime;
textFileName    = fullfile(logDir,['els_mrtrix_ainsnacc_tp100_',dateAndTime,'.csv']);
...
for i = 1:length(subs)
    subject = [subs{i}];
    subDir   = fullfile(baseDir,subject);
    dt6Dir   = fullfile(subDir,'dti60trilin');
        ...
        if exist(fiberGroup,'file')
            ...
            try
                ...
                for kk=1:size(faTP_row,1)
                    fprintf(fid1,fib, ...
                        subs{i}, fg.name,'fa', faTP_row(kk,:));
                end
                
                for kk=1:size(mdTP_row,1)
                    fprintf(fid1,fib, ...
                        subs{i}, fg.name,'md', mdTP_row(kk,:));
                end
                
                for kk=1:size(rdTP_row,1)
                    fprintf(fid1,fib, ...
                        subs{i}, fg.name,'rd', rdTP_row(kk,:));
                end
                
                for kk=1:size(adTP_row,1)
                    fprintf(fid1,fib, ...
                        subs{i}, fg.name,'ad', adTP_row(kk,:));
                end
