function fibersPDB = s_els_mrtrix_track_temp
%
...
for isubj = 1:length(subjects)
    ...
    roiFolder = fullfile(baseDir, subjectDir, 'dti60trilin/ROIs');
    ...
    maxNFibers2try = 5000; %1000000; % this is the max number of fibers to try before giving up
    ...
    for hemi = 1:length(hemis)
        fromRoiHemi = [fromRois hemis{hemi}];
        wmMaskHemi = [wmMaskFS hemis{hemi}];
        ...
        fromRoiName = fullfile(baseDir, subjectDir, '/dti60trilin/ROIs/', fromRoiHemi);
        ...
        for i_roi = 1:length(toRois)
            toRoiHemi = [toRois{i_roi} hemis{hemi}];
            toRoiName = fullfile(baseDir, subjectDir, '/dti60trilin/ROIs/', toRoiHemi);
        ...
        for nRoi = 1:length(toRois)
            ...
            roi{1} = fullfile(baseDir, subjectDir, '/dti60trilin/ROIs/', fromRoiHemi);
            % loop across toRois, must recreate hemisphere name
            toRoiHemi = [hemis{hemi} toRois{nRoi}];
            roi{2} = fullfile(baseDir, subjectDir, '/dti60trilin/ROIs/', toRoiHemi);
            ...
            roiName         = fullfile(baseDir, subjectDir, '/dti60trilin/ROIs/',[roi1.name '_' roi2.name '_union']);
