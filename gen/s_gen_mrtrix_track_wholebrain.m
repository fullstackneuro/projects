function [pdb_file, status, results] = s_gen_mrtrix_track_wholebrain(files, seed, mask, mode, n, n_max, bkgrnd, verbose)
...
% Construct file name with options
% Track, using deterministic estimate: 
tck_file = strcat(strip_ext(files.csd), '_' , ...
...
switch mode_str
  case {'SD_PROB', 'SD_STREAM'}
   ...
   cmd_str = sprintf('streamtrack -seed %s -mask %s %s %s %s -number %d -maxnum %d -stop -minlength 10 -cutoff 0.075 -initcutoff 0.05 -curvature 1',...
       seed, mask, mode_str, files.csd, tck_file, n, n_max);
