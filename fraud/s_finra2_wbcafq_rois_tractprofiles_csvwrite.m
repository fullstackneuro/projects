function s_finra2_wbcafq_rois_tractprofiles_csvwrite
%take WBC AFQ fiber group, %
...
% Set fiber groups
%fiberName = {'l_ifof_lmax10.mat'};
fiberName = {'r_ifof_lmax10.mat'};

[~,tractName, ] = fileparts(fiberName{1}); %this gets the fiber name to save in csv later
roi1Name = 'IFO_roi1_R.mat';
roi2Name = 'IFO_roi2_R.mat';
         
%% Set up the text file that will store the fiber vals.
dateAndTime     = getDateAndTime;
textFileName    = fullfile(logDir,[tractName,'wbcafq_tp100',dateAndTime,'.csv']);
...
for i = 1:numel(baseDir)
    ...
    for ii=1:numel(subs)
        ...
        if ~isempty(sub)
            ...
            roiDir   = fullfile(subDir,dirs,'ROIs'); %AFQ ROIs
            
            roi1File = fullfile(roiDir, roi1Name);
            ...
            for kk=1:numel(fiberName)
                ...
                if exist(fiberGroup,'file')
                    ...
                    try
                        ...
                        [faTP,mdTP,rdTP,adTP] = dtiComputeDiffusionPropertiesAlongFG(fg, dt, roi1,roi2, 100);
                        avgfaTP = mean(faTP);
                        avgmdTP = mean(mdTP);
                        avgrdTP = mean(rdTP);
                        avgadTP = mean(adTP);
