function s_duke_merge_wmmask_mpfc
...
for isubj = 1:length(subjects)
    ...
    for hemi = 1:length(hemis)
        roi1name = [hemis{hemi} '_wmmask_fs_fd'];
        roi2name = [hemis{hemi} '_mpfc_5mm'];
        roi1    = dtiReadRoi(fullfile(roiPath, roi1name));
        roi2    = dtiReadRoi(fullfile(roiPath, roi2name));
        newRoiName = fullfile(roiPath,[hemis{hemi} '_wmmask_fs_fd.mat']);
